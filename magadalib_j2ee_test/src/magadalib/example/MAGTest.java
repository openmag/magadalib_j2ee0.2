...
public class MAGTest extends MAGServer {
	...
	protected IMAGAuthenticator getAuthenticator() {
		return new IMAGAuthenticator() {
			...
			public String authenticate(String username, String pin, String password) {
				if(username.equals("admin") && password.equals("123")) {
					return "?_action=MAINSCREEN";
				}else {
					return null;
				}
			}
		...
		return new IMAGHandler[] {
			new IMAGHandler() {
				...
				public boolean process(MAGRequest req) {
					...
					MAGLink btn1 = new MAGLink("文档一Test", (new MAGLinkURL()).setHandler("SCREEN1"), 0);
					btn1.setNotify(true);
